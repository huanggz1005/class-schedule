...
#分班完整小班；需要完整小班助教时间和学生匹配,并尽量满足学生分到打分最高助教
Input: student_time_complete_class.xlsx, ta_time_all.xlsx
Output: complete_class_schedule.xlsx
'''
import numpy as np
import pandas as pd

student_time_path = 'data/student_time_complete_class.xlsx'
ta_time_path = 'data/ta_time_all.xlsx'
output_path = 'result/complete_class_schedule.xlsx'
import numpy as np
import pandas as pd
...
def identify_critical_timeslots(student_times):
    ...
    for student_time in student_times:
        ...
        if np.sum(student_time) <= 1:  # 1为界限，可根据实际情况调整
...
def assign_students_to_tas_with_fixed_times(ta_times, student_times, ta_free_times, max_students_per_ta=8):
    ...
    if unassigned_students:
        ...
        for student_index in unassigned_students:
            ...
            #找到所有有空的助教
            available_tas = [ta_index for ta_index in range(num_tas) if student_time[ta_free_times[ta_index]]]
            #找到学生最少的助教
            ta_index = min(available_tas, key=lambda ta_index: ta_student_count[ta_index])
            #分配学生
            assignments[student_index] = ta_index
            ta_student_count[ta_index] += 1
